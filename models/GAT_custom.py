...
class GAT(torch.nn.Module):
    def __init__(self, dropout_rate, dropedge_rate, Argument):
        ...
        if Argument.loss == 'nnet':
            self.risk_prediction_layer = PropHazards(self.postprocess.postlayernum[-1], Argument.n_intervals)
        else:
            self.risk_prediction_layer = nn.Linear(self.postprocess.postlayernum[-1], Argument.n_intervals)
