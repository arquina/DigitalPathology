...
def Train(Argument):
    ...
    checkpoint_dir = mcd(Argument)

    batch_num = int(Argument.batch_size)
    device = torch.device(int(Argument.gpu))
    metadata_root = os.path.join("/mnt/disk2/metadata/", Argument.DatasetType, Argument.CancerType)
    Metadata = pd.read_csv(os.path.join(metadata_root, Argument.CancerType + '_clinical.tsv'), sep='\t')

    TrainRoot = os.path.join("/mnt/disk2/graph_data/", Argument.DatasetType, Argument.CancerType)
    Trainlist = os.listdir(TrainRoot)
    Trainlist = [item for c, item in enumerate(Trainlist) if '_graph_torch_final.pt' in item]
    ...
    print('Train data: ', str(len(TrainFF_set[0])))
    print('Test data: ', str(len(Test_set[0])))
    print('Valid data: ', str(len(ValidFF_set[0])))
